...
def normalize_data(df, mapping, user_adjustments=None):
    ...
    if user_adjustments:
        mapping.update(user_adjustments)
    ...
    if "spend" in normalized_df.columns and "conversion_value" in normalized_df.columns and "roas" not in normalized_df.columns:
    ...
    if "spend" in normalized_df.columns and "conversions" in normalized_df.columns and "cpa" not in normalized_df.columns:
    ...
    if "clicks" in normalized_df.columns and "impressions" in normalized_df.columns and "ctr_percent" not in normalized_df.columns:
    ...
    if "spend" in normalized_df.columns and "clicks" in normalized_df.columns and "cpc" not in normalized_df.columns:
...
with tab1:
    ...
    if uploaded_file:
        ...
        st.markdown("**Szükség esetén módosítsd az oszlop-leképezést:**")
        
        correction_mode = st.radio(
            "Válassz módot:",
            ["Automata (ajánlott)", "Manuális szerkesztés"],
            horizontal=True
        )
        
        if correction_mode == "Manuális szerkesztés":
            st.markdown("#### Szerkeszt egy-egy oszlopot:")
            ...
            for csv_col in raw_df.columns:
                ...
                with col_container:
                    current_mapping = initial_mapping.get(csv_col, "")
